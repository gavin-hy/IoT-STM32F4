...
void elog_output(uint8_t level, const char *tag, const char *file, const char *func,
    ...
    extern const char *elog_port_get_p_info(void);//process info
    extern const char *elog_port_get_t_info(void);//time info
 ...
 //   char tag_sapce[ELOG_FILTER_TAG_MAX_LEN / 2 + 1] = { 0 };
    ...
    if (get_fmt_enabled(level, ELOG_FMT_LVL)) {
        log_len += elog_strcpy(log_len, log_buf + log_len, level_output_info[level]);//ok
    }
    ...
    if (get_fmt_enabled(level, ELOG_FMT_TAG)) {
        ...
        // if (tag_len <= ELOG_FILTER_TAG_MAX_LEN / 2) {//tag_len = strlen(tag) < 5
